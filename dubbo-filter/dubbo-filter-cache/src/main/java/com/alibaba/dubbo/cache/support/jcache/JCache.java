...
public class JCache implements com.alibaba.dubbo.cache.Cache {
	...
	public JCache(URL url) {
		...
		CachingProvider provider = type == null || type.length() == 0 ? Caching.getCachingProvider()
				: Caching.getCachingProvider(type);
		...
		if (cache == null) {
			try {
				...
				MutableConfiguration config = new MutableConfiguration<Object, Object>()
						...
						.setExpiryPolicyFactory(CreatedExpiryPolicy.factoryOf(new Duration(TimeUnit.MILLISECONDS,
								url.getMethodParameter(method, "cache.write.expire", 60 * 1000))))
						.setStoreByValue(false).setManagementEnabled(true).setStatisticsEnabled(true);
